...
import sys, os, statistics, math, json
...
def prompt_for_sprint_controls():
    ...
    for i, (var_name, description) in enumerate(variables.items()):
        default = defaults[i]
        while True:
            user_input = input(f"Enter number of {description} (default: {default}): ")
            if user_input == "":
...
def prompt_for_manual_corrections(story_points):
    ...
    choice = input("Do you want to make edits to the calculated story points? (Y/N): ").strip().upper()
    if choice != "Y":
        return story_points
    ...
    for category, sp_attr in story_points.items():
        ...
        for inner_key, value in sp_attr.items():
            ...
            change = input(f"Do you want to change this value? (Y/N): ").strip().upper()
            if change == 'Y':
                # Get the new value from the user
                new_value = input(f"Enter the new value for '{inner_key}': ").strip()
                try:
...
def main():
    ...
    recommendation = compute_recommendation(board, story_points, sprint_controls)

    # Insert sprint summary data into the database
    insert_sprint_summary(sprint_db_manager, board_config['board_id'], sprint_controls, story_points)
...
def compute_recommendation(board, story_points, sprint_controls):
    ...
    """
    # Calculate average unplanned and retro leftover points from the past six sprints
    ...
    length_adjustment = sprint_controls['last_sprint_days'] / sprint_controls['next_sprint_days']
    pto_adjustment = (
        sprint_controls['missed_next_sprint'] - sprint_controls['missed_last_sprint']
...
def insert_sprint_summary(sprint_db_manager, board_id, sprint_controls, story_points):
    """
